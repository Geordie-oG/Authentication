...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    if (storedItem) {
      // FIX: Create an object with ONLY the fields to update.
      // This prevents MongoDB from trying to overwrite the immutable _id field.
      const updateData = {
        name: data.name,
        price: data.price,
        amount: data.amount,
      ...
      const updatedResult = await db
        .collection("item")
        .updateOne({ _id: new ObjectId(item_id) }, { $set: updateData });
      ...
      if (updateOk)
        ...
        return errorResponse(
          { message: "Item update failed or no changes made" },
          400,
        );
    } else {
