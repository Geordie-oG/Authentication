...
export default function Item() {
  ...
  const onAddItem = async () => {
    ...
    const newItem = {
      ...
      price: price,
      amount: amount,
      status: "ACTIVE", // Automatically set status to ACTIVE on creation
    };

    const addItemResult = await fetch(`${API_URL}/api/item`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // Added header so backend reads body correctly
      },
  ...
  const onItemDelete = async (rowId) => {
    // Send a DELETE request to the dynamic route
